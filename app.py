...
def _people_table_editor(title, list_key, help_text=''):
    st.markdown(f"**{title}**")
...
def render_prvi_odjeljak():
    ...
    with dcol1:
        ...
        if statut_file:
            st.session_state.club_data["dokumenti"]["statut"] = {"name": statut_file.name, "uploaded_at": datetime.now().isoformat(timespec="seconds")}
    ...
    with dcol2:
        ...
        if other_files:
            for f in other_files:
                st.session_state.club_data["dokumenti"]["ostali"].append({"name": f.name, "uploaded_at": datetime.now().isoformat(timespec="seconds")})
    ...
    with st.container():
        st.session_state.club_data.update({
            ...
            "social": {"instagram": st.session_state.cd_instagram, "facebook": st.session_state.cd_facebook, "tiktok": st.session_state.cd_tiktok},
